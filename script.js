...
/*
  ...
  What this file does:
  - simple two-second preloader progress;
...
(() => {
  ...
  const urlLang = (() => {
    try {
      ...
      return v === 'en' || v === 'ua' ? v : null;
    } catch (e) { return null; }
  })();

  const state = {
    lang: urlLang || localStorage.getItem(CONFIG.langStorageKey) || 'ua',
    typingTimer: null,
    typingCycleId: 0,
    particles: [],
    rafId: null,
  ...
  function initPreloader() {
    ...
    setProgress(0);
    animateProgress(CONFIG.preloaderDuration, hidePreloader);
  ...
  function prepareAnimatedElements() {
    const groups = [
      ...
      { selector: '.skills-radar', effect: 'scale', stagger: 0, duration: 950 },
      { selector: '.radar-stat', effect: 'right', stagger: 60, duration: 720 },
      ...
      { selector: '.recruiter-focus', effect: 'scale', stagger: 0, duration: 900 },
      { selector: '.recruiter-focus__item', effect: 'up', stagger: 80, duration: 760 },
    ...
    $$('.about-card, .skill-block, .skills-radar, .radar-stat, .recruiter-focus, .recruiter-focus__item, .tl-item__card, .why-card, .contact-card, .btn, .tech-pill').forEach(element => {
  ...
  function setLanguage(lang, restartTyping = true) {
    ...
    if (dl) {
      const pdfHref = dl.dataset[state.lang === 'en' ? 'pdfEn' : 'pdfUa'];
      if (pdfHref) {
        dl.href = pdfHref;
        dl.setAttribute('download', pdfHref);
      }
    }
